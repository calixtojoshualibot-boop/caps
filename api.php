...
switch ($resource) {
    case 'register':
        if ($method == 'POST') {
            ...
            // Store plain text for your specific setup if not using hashing yet, 
            // but recommended to keep password_hash for security.
    ...
    case 'login':
        if ($method == 'POST') {
            ...
            foreach ($data['users'] as $user) {
                // Support both plain text (for initial admin) and hashed (for registered)
                $passMatch = ($input['password'] === $user['password']) || password_verify($input['password'], $user['password']);
                if ($user['email'] === $input['email'] && $passMatch) {
                    unset($user['password']);
